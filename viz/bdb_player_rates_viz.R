...
p1<-player_rate_metrics %>%
  dplyr::select(headshot_url,url,displayName,officialPosition,`n_NO STUNT`,
                n_STUNT,Pressure_RTOE_Delta,Block_RTUE_Delta,Def_EPA_Delta) %>%
  ...
  cols_label(
    ...
    `n_NO STUNT` = "No Stunt",
    `n_STUNT` = "Stunt",
    Block_RTUE_Delta = "Block RTUE +/-",
    Pressure_RTOE_Delta = "Pressure RTOE +/-",
  ...
  tab_header(
    title = md("**Players with biggest boost to Pressure Rates Over Expected w/ Stunts**"),
